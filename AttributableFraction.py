...
def outcome_count(df, observable, ref_IMD = "3+", ref_eth = "White"):
    count = df.groupby(["Ethnicity", "IMD"])[observable].agg([sum]).reset_index()
    count_piv = pd.pivot_table(count, values = "sum", index = "IMD", columns = "Ethnicity")
    return count_piv

def get_AF(df, observable, ref_IMD = "3+", ref_eth = "White",
           mode = "AF"):
    '''
    inputs:
    ...
    risk =  df.groupby(["Ethnicity", "IMD"])[observable].agg([np.mean, len]).reset_index()
    ref_mask = np.logical_and(risk["IMD"] == ref_IMD,
                              risk["Ethnicity"] == ref_eth)
    ...
    pivot = pd.pivot_table(risk, values = "AF_p %", index = "IMD", columns = "Ethnicity")
    
    return pivot


def new_bootstrap_AF(df, observable, ref_IMD = "3+", ref_eth = "White"):
    df = df.sample(frac=1, replace=True)
    pivot = get_AF(df, observable, ref_IMD, ref_eth)#.reset_index()
    return pivot

def get_AF_CI(df, observable, q = 0.025, n = 100):
    AF_bootstraps = np.asarray([new_bootstrap_AF(df, observable).values for i in range(n)])
    CI_vals = np.round(np.quantile(AF_bootstraps, q, axis = 0),2)
    
    CI = pd.DataFrame(CI_vals, columns=new_bootstrap_AF(df, observable).columns)
    CI["IMD"] = ["1", "2", "3+"]
...
def calc_AF(df, observable, n = 1000, ref_IMD = "3+", ref_eth = "White"):
    AF = get_AF(df, observable).reset_index()
    lower = get_AF_CI(df, observable,  q = 0.025, n=n)[list(AF.columns)]
    upper = get_AF_CI(df, observable, q = 0.975, n=n)[list(AF.columns)]
...
def calc_errors(AF):
    ...
    lower_err = (AF[0].values[:,1:] - AF[1].values[:,1:]).astype(float)
    upper_err = (AF[2].values[:,1:] - AF[0].values[:,1:]).astype(float)
    AF_errs = {"values" : mids, 
               "lower_err" : lower_err, 
               "upper_err" : upper_err, 
               "eths" : x, 
...
def plot_AF(AF_errs, outcome_counts, error_range = 100):
    ...
    ax.set_xticklabels(AF_errs["eths"], size=12)
    ax.set_yticks(range(len(AF_errs["IMD vals"])))
    ax.set_yticklabels(AF_errs["IMD vals"], size=12)
            ...
            if not ((i==len(AF_errs["eths"]) - 1) and (j==len(AF_errs["IMD vals"]) - 1)):
                ...
                if not np.isnan(AF_errs["values"][j, i]):
                    new_label = "$" + str(int(round(AF_errs["values"][j, i],0))) + \
                                 "_{+" + str(int(round(AF_errs["lower_err"][j, i],0))) + \
                                 "}^{-" + str(int(round(AF_errs["upper_err"][j, i],0))) + "}$%"
                    
                    # Add AF %
                    ax.annotate(new_label, (i, j-.1),
                                ha='center')
                    
                    n_ij = int(np.round(outcome_counts[j, i] * AF_errs["values"][j, i]/100, 0))
                    ax.annotate("[n={}]".format(n_ij), (i,j+.3), ha='center')
                else:
